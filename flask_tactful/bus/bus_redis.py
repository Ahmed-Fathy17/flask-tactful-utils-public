...
class TactfulRedisStreamBus(TactfulBus):
    ...
    def publish(self, event: Event, **send_opts) -> str:
        ...
        event.msg_id = msg_id
        self.update_docs(event)
        return msg_id
    
    def update_docs(self, event: Event):
        try:
            ...
            event_dict = event.model_dump()
            EventDocumentor.update_payload_docs(event.event, event_dict)
        except Exception as e:
            self.logger.error(f"Error occurred while updating event documentation: {e}")
